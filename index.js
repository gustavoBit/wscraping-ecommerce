...
app.get(apiPrefix + "/wsc-fidalga", (req, res) => {
  // const param = req.query.param;
  const paramUrl = req.query.requestUrl;
  console.log("paramUrl.toString()>>>>>", paramUrl.toString());

  const baseUrl = "https://www.fidalga.com";
  ...
  // WIP: pythonProcess
  const process = spawn("python3", ["wsc.fidalga.py", paramUrl]);
  process.stdout.on("data", (data) => {
    console.log(`stdout: ${data}`);
    return res.json(JSON.parse(data));
  });
...
app.get(apiPrefix + "/wsc-amarket", (req, res) => {
  // const param = req.query.param;
  const paramUrl = req.query.requestUrl;
  console.log("paramUrl.toString()>>>>>", paramUrl.toString());

  const baseUrl = "https://amarket.com.bo";
  ...
  // WIP: pythonProcess
  const process = spawn("python3", ["wsc.amarket.py", paramUrl]);
  process.stdout.on("data", (data) => {
    // console.log(`stdout: ${data}`);
    return res.json(JSON.parse(data));
  });
